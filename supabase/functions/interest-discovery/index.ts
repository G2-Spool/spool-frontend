  ...
  try {
    ...
    switch (action) {
      ...
      case 'process_message': {
        ...
        const extractionCompletion = await openai.createChatCompletion({
          model: 'gpt-4',
          messages: [
            {
              role: 'system',
          ...
          function_call: { name: 'extract_interests' }
        })
        
        const extractedData = JSON.parse(
          extractionCompletion.data.choices[0].message?.function_call?.arguments || '{}'
        )
